"""
Pilotage AUTOMATIQUE par suivi de ligne noire (capteurs FC-51).

Logique identique à follow_line.py avec les clignotants retirés
entièrement (broches GPIO, thread de clignotement, routes Flask).
La caméra n'est pas bloquante : le suivi de ligne fonctionne même
si aucune caméra n'est branchée.

Routes Flask disponibles :
  /status                      → état complet, dont battery (pourcentage entier)
  /switch/manual               → bascule vers manual_drive.py
  /control/autopilot/on        → active le suivi de ligne
  /control/autopilot/off       → met en pause le suivi de ligne
  /control/press/<cmd>         → commande manuelle (désactive l'autopilote)
  /control/release/<cmd>       → relâche une commande manuelle
  /control/throttle/<-100..100>
  /control/stop

Lancement : python auto_drive.py
"""
...
def line_follower_state() -> dict:
    ...
    return {
        ...
        "left": {
            ...
            "value":         left_value,
            "line_detected": sensor_detects_line(left_value)  if left_value  is not None else None,
        },
        "right": {
            "gpio":          RIGHT_SENSOR_PIN,
            "physical_pin":  RIGHT_SENSOR_PHYSICAL_PIN,
            "value":         right_value,
            "line_detected": sensor_detects_line(right_value) if right_value is not None else None,
        },
        "action": action,
    }
...
# ==========================================================
# CONTRÔLE MANUEL HTTP (commandes directes sur la voiture)
# ==========================================================
...
<head>
    <meta charset="UTF-8">
    <title>Voiture RC — Automatique</title>
    <style>
...
<body>
    ...
    <div class="info">État complet disponible sur <code>/status</code></div>
</body>
</html>
"""
...
# ==========================================================
# BASCULEMENT VERS LE PILOTAGE MANUEL
# ==========================================================

def _do_switch_manual():
    ...
    try:
        ...
        pass
    # Démarrer manual_drive.py comme processus indépendant
    subprocess.Popen(
        [sys.executable, str("manual_drive.py")],
...
def switch_to_manual():
    """Bascule vers le pilotage manuel."""
    threading.Thread(target=_do_switch_manual, daemon=True).start()
    return jsonify({"ok": True, "switching_to": "manual"})


def start_camera_server():
    print("Serveur RC Camera démarré (mode AUTOMATIQUE)")
...
if __name__ == "__main__":
    try:
        ...
        line_thread = threading.Thread(target=line_follower_loop, daemon=True)
        line_thread.start()
